...
func TestCompilerScopes(t *testing.T) {
	...
	if last.Opcode != code.OpSub {
		t.Errorf("lastInstruction.Opecode wrong. got=%d, want=%d",
...
func testConstants(
	...
	for i, constant := range expected {
		...
		case int:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testIntergerObject failed: %s",
					i, err)
			}
		...
		case []code.Instructions:
			...
			if !ok {
				return fmt.Errorf("constant %d - not a functions: %T",
					i, actual[i])
			}

			err := testInstructions(constant, fn.Instructions)
			if err != nil {
				return fmt.Errorf("constant %d - testInsructions failed: %s",
					i, err)
			}
		}
	}
...
func testIntegerObject(expected int64, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object has wrong value. got=%T (%+v)",
